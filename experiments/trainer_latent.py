...
def get_dataset(args, tokenizer, text_encoder, vqvae, accelerator):
    ...
    with torch.no_grad():
        df["images"] = 1 / 0.18215 * torch.stack(images, dim=0)

    df["text_embeddings"] = embed(
        df["description"], tokenizer, text_encoder, accelerator
    )

    with torch.no_grad():
        df["latents"] = accelerator.unwrap_model(vqvae).encode(df["images"]).mode()
        ...
        with tqdm(total=len(train_dataloader), unit="ba") as pbar:
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                timesteps = torch.randint(
                    0, noise_scheduler.timesteps, (bsz,), device=clean_latents.device
                ).long()

                # add noise onto the clean latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                noisy_latents = noise_scheduler.training_step(
                    clean_latents, noise_samples, timesteps
                )

                if step % args.gradient_accumulation_steps != 0:
                    with accelerator.no_sync(model):
                        output = model(
                            noisy_latents, timesteps, context=batch["text_embeddings"]
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_path", type=str, default="fusing/dog_captions")
